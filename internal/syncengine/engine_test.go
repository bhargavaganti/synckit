...
func mkBundle(host, app string, t time.Time) service.Bundle {
...
func TestComputeUpdates(t *testing.T) {
	...
	ov := service.Overview{
		LocalBundles: []service.Bundle{
			mkBundle(me, "chrome", now.Add(-2*time.Hour)), // our chrome watermark
			mkBundle(me, "firefox", now),                  // our firefox is current
		},
		Peers: []service.Peer{
			{Host: "laptop", IP: "100.0.0.2", Online: true, Serving: true, Bundles: []service.Bundle{
				mkBundle("laptop", "chrome", now),             // newer chrome → update
				mkBundle("laptop", "firefox", now.Add(-time.Hour)), // older firefox → no
			}},
			{Host: "self-echo", IP: "100.0.0.3", Online: true, Serving: true, Bundles: []service.Bundle{
				mkBundle(me, "chrome", now.Add(time.Hour)), // our own bundle on a peer → ignore
			}},
			{Host: "offline", IP: "100.0.0.4", Online: false, Serving: false, Bundles: []service.Bundle{
				mkBundle("offline", "dbeaver", now.Add(time.Hour)), // not serving → ignore
			}},
		},
	}
...
func TestComputeUpdates_NoneWhenLocalNewer(t *testing.T) {
	...
	me := "desktop"
	ov := service.Overview{
		LocalBundles: []service.Bundle{mkBundle(me, "chrome", now)},
		Peers: []service.Peer{{Host: "laptop", IP: "1", Online: true, Serving: true,
			Bundles: []service.Bundle{mkBundle("laptop", "chrome", now.Add(-time.Hour))}}},
	}
